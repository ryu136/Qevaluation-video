...
Usage: $(basename "$0") [option] YUVfile (EncodedFile)
  ...
  -r VALUE    Frame rate
  -c String   Used codec by encoding
EOM
...
#echo ffmpeg -i $ref_file -i $encode_file  -filter_complex psnr=../log/psnr_stats_$DATE.log -an -f null - 

# ssim
echo "SSIM"
ffmpeg -i $ref_file -i $encode_file  -filter_complex ssim=../log/ssim_stats.log -an -f null - >& ../log/ssim_avg_$DATE.log
#echo ffmpeg -i $ref_file -i $encode_file  -filter_complex ssim=../log/ssim_stats_$DATE.log -an -f null - 
